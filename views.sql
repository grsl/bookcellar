/*
|---------------------------------------
| Views on the Bookcellar Database.
|---------------------------------------
|
*/
    CREATE VIEW `vAuthors` AS
        SELECT
            CONCAT(
                IF((`authors`.`givenname` <> ""), CONCAT(`authors`.`givenname`,' '),'')
               ,IF((`authors`.`initials`  <> ""), CONCAT(`authors`.`initials`,' '),'')
...
*/
    ...
    CREATE VIEW `vOrders` AS
        SELECT `o`.`order_id`   AS "ID"
              ...
              ,CONCAT(
                  IF((`u`.`givenname` <> ""), CONCAT(TITLE(`u`.`givenname`),' '),'')
                 ,IF((`u`.`initials`  <> ""), CONCAT(TITLE(`u`.`initials`),' '),'')
                 ,`u`.`familyname`) AS "Name"
             #Home Address.
             ,CONCAT(
                  CONCAT(IF(`a`.`house_no`   <> "", CONCAT(`a`.`house_no`, ", "), ""))
                 ,CONCAT(IF(`a`.`house_name` <> "", CONCAT(`a`.`house_name`, ", "), ""))
                 ,`a`.`street`, ", "
                 ,`a`.`district`, ", "
                 ,`a`.`county`, ", "
                 ,`c`.`name`, ".") AS "Address"
             #Billing Address.
             ,CONCAT(
                  CONCAT(IF(`ba`.`house_no`   <> "", CONCAT(`ba`.`house_no`, ", "), ""))
                 ,CONCAT(IF(`ba`.`house_name` <> "", CONCAT(`ba`.`house_name`, ", "), ""))
                 ,`ba`.`street`, ", "
                 ,`ba`.`district`, ", "
                 ,`ba`.`county`, ", "
             ...
             #Delivery Address.
             ,CONCAT(
                  CONCAT(IF(`da`.`house_no`   <> "", CONCAT(`da`.`house_no`, ", "), ""))
                 ,CONCAT(IF(`da`.`house_name` <> "", CONCAT(`da`.`house_name`, ", "), ""))
                 ,`da`.`street`, ", "
                 ,`da`.`district`, ", "
                 ,`da`.`county`, ", "
